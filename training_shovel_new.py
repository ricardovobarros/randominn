...
files = "C:\\Users\\ricar\\MA\\training_csv\\shovel_4classes_corrected"
...
# df_train = modify_predictors_labels_shovel(df=df_train)

# #  save for posterior analysis (not import in the code)
# df_train.to_csv("Testfile/training_set_example.csv", index=False)


# define the list to be droped
list_drop = ["class", "x", "y", "name"] #,"wa" ,"lateral","tri","rgb","longitudinal","green","vari"
...
for i in range(0, n_runs):
    ...
    print()
if n_runs != 1:
    pd.DataFrame(precision_matrix).to_csv("training_csv/sensitivity_new/shovel_newparameters.csv", index=False)
...
# # __________________plot improvement of oob or test set increasing the number og threes_______________________
...
# xsdum, ys_train = zip(*error_rate_train)
# fig, ax1 = plt.subplots()
# ax1.plot(xs, ys, label="mean validation error", marker="o")
# ax1.plot(xs, ys_train, label="mean train error", marker="o")
# # plt.xlim(min_depth, max_depth)
...
labels = np.array(["Courser Cobble", "Gravel Pred.",  "Finer Cobble", "Sand"])
plot_confusion_matrix(rfc, X_test, y_test, display_labels=np.array(labels), normalize="true")
plt.show()
#
...
print("testing score:", accuracy_score(y_test, rfc_predict))
print()
print("trainset:", df_train_set["name"].unique())
print()
print("dataset:", df_save["name"].unique())
# print(oob_best_iter)
